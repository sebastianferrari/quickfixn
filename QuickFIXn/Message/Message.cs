...
namespace QuickFix
{
	...
	public class Message : FieldMap
	{
			...
			: base(src)
		{
			...
			validStructure_ = src.validStructure_;
			field_= src.field_;
		}
		...
		{
			return (
				(Header.CalculateTotal(Encoding)
				+ CalculateTotal(Encoding)
				+ Trailer.CalculateTotal(Encoding)) % 256);
		}

		public bool IsAdmin()
		{
			return Header.IsSetField(Tags.MsgType) && IsAdminMsgType(Header.GetField(Tags.MsgType));
		}
		...
		{
			return Header.CalculateLength(Encoding) + CalculateLength(Encoding) + Trailer.CalculateLength(Encoding);
		}
	}
}